...
module BvhNode3d =
    ...
    let rec getIntersectingTail (current : HashMap<'K, struct (Box3d * 'V)>) (query : Box3d) (node : BvhNode3d<'K, 'V>) =
        match node with
        | Leaf(_, bounds, values) ->
            if bounds.Intersects query then
                let res = values |> HashMap.filter (fun _ struct(b,_) -> b.Intersects query)
                if res.Count = 0 then 
                    current
                elif res.Count = 1 then     
                    let mutable current = current
                    res |> HashMap.iter (fun k v ->
                        current <- HashMap.add k v current
                    )
                    current
                else
                    HashMap.union current res
            else
                current
        | Node(_bestCost, _cnt, bounds, l, r) ->
            if bounds.Intersects query then
                let l = getIntersectingTail current query l
                getIntersectingTail l query r
            else
                current
           
    let getIntersecting (query : Box3d) (node : BvhNode3d<'K, 'V>) =
        getIntersectingTail HashMap.empty query node
